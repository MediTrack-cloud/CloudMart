...
| VPC Interface Endpoints | secretsmanager, ecr.api, ecr.dkr (3 × 2 AZ) | $0.01/AZ-hr | 6 | $44 |
| **Total (prod)** | | | | **~$416/month** |

> S3 and DynamoDB use **gateway** endpoints (no hourly charge). The three **interface** endpoints are
> deployed in both AZs (`infra/modules/vpc/main.tf`), so they bill as 6 ENI-hours — the same in staging.
...
|-----------------|--------|
| Single NAT Gateway (1 vs 2) | –$33 |
| RDS db.t3.micro Single-AZ (vs db.t3.small Multi-AZ) | –$40 |
| **Total (staging)** | **~$343/month** |

> Staging runs the **same 2-node group** (`eks_min_nodes = 2` in `infra/environments/staging/terraform.tfvars`)
> and the **same three interface endpoints** as prod — there is no node-count or endpoint saving. Pod-level
> cost is reduced instead: the staging Kustomize overlay scales frontend/product/order to **1 replica each**.
...
| Layer | Cost | % |
|-------|------|---|
| Compute (EKS + EC2) | $21.30 | 51% |
| Database (RDS + DynamoDB) | $5.70 | 14% |
| Network (NAT + ALB) | $8.30 | 20% |
| Other (WAF, Secrets, KMS, logs, endpoints, GuardDuty, storage) | $6.30 | 15% |
| **Total** | **$41.60** | 100% |

**Scale efficiency:** At 50 000 orders/month (5× scale), fixed costs dominate → ~$8.32/1K orders (5× improvement).
